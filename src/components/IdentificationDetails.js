...
const IdentificationDetails = (props) => {
  ...
  const validate = () => {
    ...
    if (
      (!pattern.test(empId) && !pattern1.test(empId) && empId != '') ||
      (pattern1.test(empId) && empId == '') ||
      !pattern1.test(empId)
    ) {
    ...
    if (
      (!pattern.test(SSN) && !pattern1.test(SSN) && SSN != '') ||
      (pattern1.test(SSN) && SSN == '') ||
      !pattern1.test(SSN)
    ) {
      setIsValidSSN(false);
      errors.push('SSN');
    ...
    if (
      (!pattern.test(CSSN) && !pattern1.test(CSSN) && CSSN != '') ||
      (pattern1.test(CSSN) && CSSN == '') ||
      !pattern1.test(CSSN)
    ) {
    ...
    if (
      (!pattern.test(AINNo) && !pattern1.test(AINNo) && AINNo != '') ||
      (pattern1.test(AINNo) && AINNo == '') ||
      !pattern1.test(AINNo)
    ) {
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.container1}>
        ...
        <TextInput
          ...
          right={
            <TextInput.Icon name="information" color="rgba(34, 42, 53, 0.6)" />
          }
        />

        {!isValidEmpId ? (
          <HelperText type="error">Employee Id is invalid!</HelperText>
        ) : null}
        <TextInput
          style={styles.inputStyle}
        ...
        {!isValidDOB ? (
          <HelperText type="error">Date of birth is invalid!</HelperText>
        ) : null}
        ...
        <TextInput
          ...
          placeholderTextColor="rgba(34, 42, 53, 0.6)"
          keyboardType="numeric"
          secureTextEntry={showSSN ? false : true}
          value={SSN}
          onChangeText={(e) => {
            setSSN(e);
            setIsValidSSN(true);
        ...
        {!isSameSSN ? (
          <HelperText type="error">
            Social Security Number donot match with Confirm Social security
            Number
          </HelperText>
        ) : null}
        ...
        <TextInput
          ...
          placeholderTextColor="rgba(34, 42, 53, 0.6)"
          keyboardType="numeric"
          secureTextEntry={showCSSN ? false : true}
          value={CSSN}
          onChangeText={(e) => {
            setCSSN(e);
            setIsValidCSSN(true);
          ...
          right={
            <TextInput.Icon
              name={showCSSN ? 'eye' : 'eye-off'}
              color="rgba(34, 42, 53, 0.6)"
              onPress={() => setShowCSSN(!showCSSN)}
            />
          }
        />
        ...
        {!isSameSSN ? (
          <HelperText type="error">
            COnfirm Social Security Number donot match with Social security
            Number
          </HelperText>
        ) : null}
        ...
        <TouchableOpacity
          ...
          onPress={() => setAINModal(!AINModal)}
        >
          <View style={styles.iconContainer}>
            <Text style={styles.textcenter}>{AINType}</Text>
            <AntDesign
              name="down"
              size={14}
              color="rgba(34, 42, 53, 0.6)"
          ...
          >
            <View style={styles.centeredView}>
              <View style={styles.modalView}>
                ...
                <TouchableOpacity
                  style={
                    AINType == 'Driving License'
                      ? styles.modalOptionTrue
                      : styles.modalOptionDefault
                  }
                  onPress={() => {
                ...
                >
                  <Text
                    style={
                      AINType == 'Driving License'
                        ? styles.modalOptionTextTrue
                        : styles.modalOptionTextDefault
                    }
                  >
                    Driving License
                  </Text>
        ...
        <View>
          {showLicense ? (
            <View style={{ marginLeft: 30 }}>
              <TextInput
                style={styles.inputStyle}
                placeholder="State of Issusance"
                placeholderTextColor="rgba(34, 42, 53, 0.6)"
                value={stateIssue}
                onChangeText={(e) => {
                  setStateIssue(e);
              ...
              {!isValidStateIssue ? (
                <HelperText type="error">State is invalid!</HelperText>
              ) : null}
              <TextInput
                style={styles.inputStyle}
        ...
        <Text style={styles.subHeading}>
          Are you currently participating in a bankruptcy proceeding
        </Text>
        <TouchableOpacity
          style={styles.buttonContainer1}
          onPress={() => setBCModalVisible(!bcModalVisible)}
        >
          <View style={styles.iconContainer}>
            <Text style={styles.textcenter}>{bankType}</Text>
            <AntDesign
              name="down"
              size={14}
              color="rgba(34, 42, 53, 0.6)"
        ...
        {!isValidBC ? (
          <HelperText type="error">bankruptcy is invalid!</HelperText>
        ) : null}
        ...
        {!isValidTerms ? (
          <HelperText type="error">Terms and conditions is invalid!</HelperText>
        ) : null}
      </ScrollView>
      <View style={styles.buttonContainer}>
        <TouchableOpacity style={styles.backButton}>
          <Text style={styles.buttonText} onPress={() => previousForm()}>
            Back
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
